...
const ServiceDetail = () => {
  ...
  if (!service) {
    return <div style={{textAlign:'center', margin:'3rem 0', fontSize:'1.3rem', color:'var(--color-accent)'}}>الخدمة غير موجودة</div>;
  }

  return (
    <section style={{maxWidth:'700px', margin:'3rem auto', background:'var(--color-muted)', borderRadius:'1.5rem', padding:'2.5rem 1.5rem', textAlign:'center'}}>
      {service.icon}
      <div style={{fontWeight:'bold', fontSize:'2rem', marginBottom:'1.2rem', color:'var(--color-accent)'}}>{service.title}</div>
      ...
      <div style={{fontSize:'1.15rem', color:'var(--color-accent)', lineHeight:'2', marginBottom:'1.5rem'}}>{service.desc}</div>
      {service.more && (
        <ul style={{listStyle: 'none', padding: 0, margin: 0}}>
          {service.more.map((p, idx) => (
            <li key={idx} style={{fontSize:'1.05rem', color:'var(--color-text)', background:'var(--color-muted)', borderRadius:'0.7rem', padding:'1rem', margin:'1rem 0'}}>{p}</li>
          ))}
        </ul>
      )}
    </section>
