...
const ServiceDetail = () => {
  ...
  if (!service) {
    return <div style={{textAlign:'center', margin:'3rem 0', fontSize:'1.3rem', color:'#003366'}}>الخدمة غير موجودة</div>;
  }

  return (
    <section style={{maxWidth:'700px', margin:'3rem auto', background:'#FFF9E3', borderRadius:'1.5rem', boxShadow:'0 4px 24px rgba(0,0,0,0.10)', padding:'2.5rem 1.5rem', textAlign:'center', border:'1px solid #FFE066'}}>
      {service.icon}
      <div style={{fontWeight:'bold', fontSize:'2rem', marginBottom:'1.2rem', color:'#003366'}}>{service.title}</div>
      ...
      <div style={{fontSize:'1.15rem', color:'#222', lineHeight:'2', marginBottom:'1.5rem'}}>{service.desc}</div>
      {service.more && (
        <ul style={{listStyle: 'none', padding: 0, margin: 0}}>
          {service.more.map((p, idx) => (
            <li key={idx} style={{fontSize:'1.05rem', color:'#003366', background:'#fff', borderRadius:'0.7rem', padding:'1rem', margin:'1rem 0', boxShadow:'0 1px 4px rgba(0,0,0,0.04)'}}>{p}</li>
          ))}
        </ul>
      )}
    </section>
